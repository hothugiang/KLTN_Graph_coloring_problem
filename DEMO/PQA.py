...
def sample_bqm(bqm_combined, bqm_list, graphs, ilp_colors, embeddings, sampler, annealing_time, num_reads=1000):
    ...
    for graph_id, stat in stats.items():
        ...
        res = {
            ...
            "P_sol": P_sol,
            "max_constrain": stat['max_constrain_violation']
        }
...
def main(graphs, ilp_colors, chain_strength, annealing_time):
    alpha = 1
    beta = 1
    token = 'DEV-54b2c12e1e296785c2205419b243b7ecc734fb75'
...
if __name__ == "__main__":
    ...
    for x in range(1, 4):  
        ...
        graph1, chroma1 = read_graph_from_file(f"Test/5_{x}.txt")
        graph2, chroma2 = read_graph_from_file(f"Test/10_{x}.txt")
        graph3, chroma3 = read_graph_from_file(f"Test/15_{x}.txt")
        graph4, chroma4 = read_graph_from_file(f"Test/20_{x}.txt")
        graphs = [
            graph1,
            # graph2,
            graph3,
    ...
    with open(f'Res/ilp+1.json', 'w') as f:
