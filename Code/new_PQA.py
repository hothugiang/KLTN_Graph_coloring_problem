...
def sample_bqm(bqm_combined, sampler, annealing_time, num_reads=1000):
...
def parallel_annealing(alpha, beta, token, graphs, chain_strength, annealing_time):
    ...
    result = sample_bqm(bqm_combined, SimulatedAnnealingSampler, annealing_time)
...
def interpret_and_check_results(result, embeddings, bqm_list, graphs):
    interpreted_results = []
    ...
    for graph_id, (bqm, embedding) in enumerate(zip(bqm_list, embeddings)):
        ...
        for variable in bqm.variables:
            if variable.startswith('q_') and variable in embedding:  # Bỏ qua các biến 'color_used' và chỉ xét 'q_'
                ...
                # Nếu có nhiều qubit, ta lấy kết quả của qubit đầu tiên (vì tất cả qubit trong chain nên giống nhau)
                for qubit in qubits:
        ...
        # H1: Kiểm tra mỗi đỉnh đều được tô một màu duy nhất
        ...
        # H2: Kiểm tra các đỉnh kề nhau không có cùng màu
        if valid:
        ...
        # Gọi hàm ilp_graph_coloring để tìm số màu tối thiểu sử dụng
        ilp_num_colors = ilp_graph_coloring(graph)

        # Kiểm tra tính hợp lệ của kết quả từ QA với kết quả từ ILP
        success = 0
        if valid and len(used_colors) == ilp_num_colors:
            success = 1

        # Thêm kết quả vào interpreted_results
        interpreted_results.append({
            # "graph_id": graph_id,
            "node_color_mapping": sorted_node_color_mapping,
            "num_colors_used": len(used_colors),
            "ilp_num_colors": ilp_num_colors,  # Thêm kết quả từ ILP
            "valid": valid,
            "constraint_violations": constraint_violations,
            "success": success  # Đánh dấu nếu kết quả hợp lệ và số màu đúng
        })

    return interpreted_results


def main(graphs, chain_strength, annealing_time):
    alpha = 1
    beta = 1
    ...
    bqm_list = [create_bqm_for_graph(graph, alpha, beta, graph_id) for graph_id, graph in enumerate(graphs)]
    results, embeddings = parallel_annealing(alpha, beta, token, graphs, chain_strength, annealing_time)
    
    if results:
        info = results.get('info')
        interpreted_results = interpret_and_check_results(results, embeddings, bqm_list, graphs)
...
if __name__ == "__main__":
    for N in [20]:
        ...
        for annealing_time in [5, 10, 15, 20, 25, 30]:
            print(f'annealing_time: ', annealing_time)
            start_time_chain = time.time()
            res = []
            for x in range(1, 31):    
                graphs = [
                    read_graph_from_file(f"Test/final/{N}_{x}.txt"),
                    # read_graph_from_file(f"Test/2111/10_2.txt"),
                ]
                print(f'{N}_{x}')
                
                # Lưu kết quả của mỗi lần chạy vào all_results
                info, interpreted_results = main(graphs, chain_strength, annealing_time)
                res.append({
                    "Graph": f'{N}_{x}',
                    "info": info,
        ...
        with open(f'Res/final/find_annealing_time/n={N}_QA.json', 'w') as f:
