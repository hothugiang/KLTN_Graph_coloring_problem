...
def parallel_annealing(alpha, beta, token, graphs, ilp_colors, chain_strength, annealing_time):
    ...
    result = sample_bqm(bqm_combined, bqm_list, graphs, ilp_colors, [embedding for embedding, _ in embeddings], hardware_sampler, annealing_time)
...
def main(graphs, ilp_colors, chain_strength, annealing_time):
    alpha = 1
    beta = 1
    # token = 'DEV-f4324526c6a88262c654330f3a9bad5c4baf238b'
    token = 'DEV-2beb22659188dfbe577d61d2312143c6b6415226'
...
if __name__ == "__main__":
    ...
    start_time = time.time()
    res = []
    for x in range(1, 26, 4):   
        problem = f'(20_{x}, 20_{x+1}, 20_{x+2}, 20_{x+3})'
        print(problem) 
        graph1, chroma1 = read_graph_from_file(f"Test/final/20_{x}.txt")
        graph2, chroma2 = read_graph_from_file(f"Test/final/20_{x+1}.txt")
        graph3, chroma3 = read_graph_from_file(f"Test/final/20_{x+2}.txt")
        graph4, chroma4 = read_graph_from_file(f"Test/final/20_{x+3}.txt")
        # graph2, chroma2 = read_graph_from_file(f"Test/final/10_{x}.txt")
        # graph3, chroma3 = read_graph_from_file(f"Test/final/15_{x}.txt")
        # graph4, chroma4 = read_graph_from_file(f"Test/final/20_{x}.txt")
        graphs = [
            graph1,
            graph2,
            graph3,
    ...
    with open(f'Res/final/combine_problem/QA/4/(20, 20, 20, 20).json', 'w') as f:
